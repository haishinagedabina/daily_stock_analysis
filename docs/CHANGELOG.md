...
- 🤖 **Feishu bot config completeness** — added explicit config-schema support for `FEISHU_STREAM_ENABLED`, `FEISHU_VERIFICATION_TOKEN`, and `FEISHU_ENCRYPT_KEY`; `.env.example` now documents Feishu Stream mode with placeholders instead of embedded real credentials, and local setup docs now clarify how to enable app-bot mode without a public callback URL
- 📈 **Market screening backend progression** — formalized `instrument_master`, `daily_factor_snapshot`, market-data sync service, factor snapshot persistence, and staged screening orchestration; planning docs now include local retention/cleanup strategy for SQLite data, logs, and AI artifacts
- ⚙️ **Market screening config integration** — added explicit `SCREENING_*` config fields for candidate limits, AI Top K, rule thresholds, and factor/breakout lookbacks; Web config schema now exposes these fields, runtime validation prevents `AI_TOP_K > CANDIDATE_LIMIT`, and screening services/API now consume config defaults when request values are omitted
- 🎛️ **Market screening mode presets** — added `SCREENING_DEFAULT_MODE` plus built-in `balanced` / `aggressive` / `quality` presets; screening runs can now switch modes per request, runtime services rebuild with mode-specific thresholds, and run detail/list payloads expose the resolved mode and effective snapshot
- ♻️ **Market screening health and rerun controls** — daily sync now returns a concise `health_report` with availability, missing codes, success rate, and error counts; screening runs now avoid duplicate same-day same-mode tasks, allow failed-run reruns from supported stages, and `POST /api/v1/screening/runs` returns the persisted run resource even when execution status is `failed`
- 🧠 **Market screening AI/news recommendation layer** — candidate AI analysis now supports limited top-M news enrichment, screening runs can finish as `completed_with_ai_degraded` when AI enrichment fails without blocking rule results, and candidate list responses now expose derived recommendation fields such as `final_rank`, `final_score`, `recommendation_source`, `recommendation_reason`, and aggregated news summary/count
- 🔎 **Market screening candidate detail API** — added per-candidate detail retrieval under the `screening` namespace, including rule-hit context, AI summary fields, and linked `analysis_history` metadata for deep-analysis drilldown without querying the database directly
