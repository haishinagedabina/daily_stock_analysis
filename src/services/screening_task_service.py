...
class ScreeningTaskService:
    ...
    def execute_run(
        ...
        sync_warning_summary: Optional[str] = None
        ...
        try:
            ...
            if resume_stage != "factorizing":
                ...
                ignored_sync_errors, blocking_sync_errors = self._partition_sync_errors(
                    universe_df=universe_df,
                    errors=sync_result.get("errors", []),
                )
                if ignored_sync_errors:
                    ignored_codes = [str(item.get("code", "")).strip().upper() for item in ignored_sync_errors]
                    sync_warning_summary = self._build_ignored_sync_error_summary(ignored_sync_errors)
                    universe_df = universe_df[
                        ~universe_df["code"].astype(str).str.upper().isin(ignored_codes)
                    ].reset_index(drop=True)
                    if universe_df.empty:
                        raise ValueError(
                            self._merge_error_summary(
                                sync_warning_summary,
                                "剔除退市且无数据股票后，股票池为空，无法继续筛选",
                            )
                        )
                ...
                self._log_stage_completed(
                    ...
                    ignored_error_count=len(ignored_sync_errors),
                )
    ...
    def get_run(self, run_id: str) -> Optional[Dict[str, Any]]:
        return self.db.get_screening_run(run_id)

    def list_runs(self, limit: int = 20) -> List[Dict[str, Any]]:
        return self.db.list_screening_runs(limit=limit)
    ...
    @classmethod
    def _partition_sync_errors(cls, universe_df: Any, errors: List[Dict[str, Any]]) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        if not errors:
            return [], []
        ...
        for item in errors:
            ...
            if cls._is_ignorable_sync_error(item, universe_map.get(code, {})):
                ignored.append(item)
            else:
